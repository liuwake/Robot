...
vector <vector<float>> initialize_beliefs(vector <vector<char>> grid) {
    ...
    vector <vector<float>>
            newGrid(grid.size(), vector<float>(grid[0].size(), prob));
    // your code here
...
vector <vector<float>> move(int dy, int dx,
    ...
    vector <vector<float>>
            newGrid(beliefs.size(), vector<float>(beliefs[0].size(), 0.0));
    // your code here
    for (size_t r = 0; r < beliefs.size(); r++) {
        for (size_t c = 0; c < beliefs[0].size(); c++) {
            newGrid[r][c] =
                    beliefs[(r - dy + beliefs.size()) % beliefs.size()]
                    [(c - dx + beliefs[0].size()) % beliefs[0].size()];
        }
    }
